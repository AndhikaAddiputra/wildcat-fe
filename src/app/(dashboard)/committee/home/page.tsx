...
export default function CommitteeStatisticsPage() {
  ...
  // Fungsi loadData tetap menggunakan no-store agar selalu mendapat data paling update
  const loadData = async () => {
    setIsLoading(true);
    try {
      const meRes = await fetchWithAuth("/api/admin/me", { cache: "no-store" });
      if (meRes.ok) {
        const meData = await meRes.json();
        if (meData.success && meData.committee?.name) {
          setCommitteeName(meData.committee.name);
        }
      }
  ...
  return (
    <div className="min-h-screen w-full font-['Poppins']">
      <div className="relative min-h-screen bg-[#0A2D6E]">
        ...
        <main className="relative z-10 mx-auto max-w-6xl px-6 pt-32 pb-20">
          <section className="mb-10">
            <h1 className="text-4xl font-bold text-[#F6911E] sm:text-5xl">Hello, {committeeName}!</h1>
          ...
          <CardLarge className="p-8 bg-[#0A2D6E]/90 border-[#F6911E]/30 backdrop-blur-md">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              {safeCompetitions.length > 0 ? safeCompetitions.map((comp: { competitionName?: string; teamCount?: number; participantCount?: number }, idx: number) => (
                <div key={idx} className="bg-[#3c3c9c] p-6 rounded-2xl border border-white/10 text-center">
                  ...
                  <p className="text-3xl font-bold text-[#F6911E]">{comp.teamCount ?? 0}</p>
                  <p className="text-[#F6911E]/80 text-xs">Teams</p>
                </div>
              )) : (
            ...
            <div className="space-y-4 mb-10">
              {safeEvents.length > 0 ? safeEvents.map((event: { name?: string; registeredCount?: number; attendedCount?: number }, idx: number) => (
                <div key={idx} className="bg-[#3c3c93] p-5 rounded-2xl flex flex-col md:flex-row items-center justify-between border border-white/5">
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10 w-full md:w-auto">
                    ...
                    <div className="flex items-center justify-end gap-3 w-full md:w-32">
                      ...
                      {/* 🌟 TOMBOL EDIT DIHAPUS DI HALAMAN COMMITTEE */}
                    </div>
                  </div>
                </div>
              )) : (
            ...
            <div className="flex flex-col mb-10 md:flex-row justify-between items-center gap-6">
              <div className="flex flex-col md:flex-row gap-4 md:gap-6 text-[10px] sm:text-xs text-[#F6911e]">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/* 🌟 TEKS KETERANGAN EDIT DIHAPUS */}
                  <span>Participants Present (Contact Admin to Edit Manually)</span>
                </div>
              </div>
            </div>
      ...
      <Footer />
      {/* 🌟 KODE MODAL POP UP SEPENUHNYA DIHAPUS DARI SINI */}
    </div>
  );
}